...
The *align* function within **Rsubread** was used to perform the alignment. Gene annotation files from Ensembl, GENCODE, RefSeq-NCBI and the inbuilt RefSeq annotation in Rsubread were used as part of the alignment. 
...
[GENCODE release 34](ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_34/gencode.v34.annotation.gtf.gz)
...
annotation.files <- list(
  ...
  GENCODE = "gencode.v34.annotation.gtf",
...
### Comparison of genomic features in Ensembl, GENCODE and RefSeq 
...
prop.counts <-
  subset(summary.data,
         subset = Metric == "PropSummarized" & Annotation != "GENCODE")
...
The similarities in the complexity between Ensembl (v100), GENCODE (v34) and the 2 RefSeq (P13 for the public version and P1 for the Rsubread version) annotations are compared below.
```{r}
if(!require(tidyr))
...
knitr::kable(
  ...
  align = "c",
  caption = "Comparison of features in Ensembl, GENCODE, RefSeq-NCBI and RefSeq-
Subread"
)
```
...
#Exclude GENCODE
annotations<-annotations[!grepl("GENCODE",annotations)]
